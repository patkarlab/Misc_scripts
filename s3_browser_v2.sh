...
do 
	...
	do 
		...
		if grep "${folder_path}" ${fastq_file_csv}_s3_files.dat ; then
			repeat_folder=repeat_`date "+%d%m%y_%H%M%S"`
			echo "File $fastq_file_name is already present in the destination, moving it to ${folder_name}/${panel_dir}/${repeat_folder}" >> ${fastq_file_csv}.info
			mkdir -p ${folder_name}/${panel_dir}/${repeat_folder}
			mv ${folder_name}/${panel_dir}/${sample_name_full}_S*fastq* ${folder_name}/${panel_dir}/${repeat_folder}/
		       	break
		fi
	done
	...
	fastq_files=$(find ${folder_name} -type f -iname "*fastq.gz")
...
#aws s3 cp s3://hematopath-data/Clinical_NGS_Analysis/01FastqArchival2016/ s3://hematopath-data/Clinical_NGS_Analysis/01FastqArchival2016/ --storage-class GLACIER_IR --recursive
